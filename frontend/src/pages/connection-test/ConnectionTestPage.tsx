...
function getErrorDetails(error: unknown) {
  if (error instanceof ApiError) {
    ...
    return {
      ...
      description:
        backendMessage ||
        "The backend returned an unsuccessful readiness response.",
      statusCode: error.status,
      messageKey:
        typeof error.body.messageKey === "string"
          ? error.body.messageKey
          : null,
    };
  }
  ...
  return {
    title: "Backend readiness check failed.",
    description:
      "An unexpected error happened while checking backend readiness.",
    statusCode: null,
    messageKey: null,
  };
}
...
export default function ConnectionTestPage() {
  const readinessQuery = useQuery({
    ...
    queryFn: () =>
      get<ReadinessResponse>("/api/health/ready", { locale: "en" }),
    retry: false,
    staleTime: 0,
  });
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-xs font-semibold text-[#98A2B3]">
                Endpoint:{" "}
                <span className="font-black text-[#344054]">
                  /api/health/ready
                </span>
              </p>
            </div>
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                  {overviewItems.length > 0 ? (
                    ...
                    <div className="rounded-[16px] border border-dashed border-[#D0D5DD] bg-white p-4 text-sm text-[#667085] md:col-span-2 xl:col-span-5">
                      The readiness endpoint responded successfully, but it did
