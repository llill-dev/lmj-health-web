...
function accountStatusPresentation(
  ...
  if (status === "temporary" || patient.isTemporary) {
    return {
      label: t("secretary.patients.accountStatus.temporary"),
  ...
  if (status === "suspended") {
    return {
      label: t("secretary.patients.accountStatus.suspended"),
      className: "bg-[#FEE2E2] text-[#B42318]",
    };
  }

  return {
    label: t("secretary.patients.accountStatus.active"),
    className: "bg-[#ECFDF3] text-[#16A34A]",
  };
}

function SurfaceSection({ children }: { children: React.ReactNode }) {
  return (
  ...
  return (
    <div className="relative min-w-0">
      <input
        ...
        aria-label={t("secretary.patients.searchPatient")}
...
const PatientTableRow = memo<{
  ...
  locale: "ar" | "en";
  t: (key: string) => string;
}>(function PatientTableRow({ patient, expanded, onToggle, locale, t }) {
    ...
    <div className="border-b border-[#EEF2F6] last:border-b-0">
      <div className="grid grid-cols-1 gap-4 px-4 py-4 sm:px-6 lg:grid-cols-12 lg:items-center lg:px-8 lg:py-5">
        <div className="flex gap-4 items-center lg:col-span-4">
          ...
          <div className="min-w-0 text-start">
            ...
            <div className="truncate font-cairo text-[13px] font-bold text-[#98A2B3]">
              {t("secretary.patients.fileNumber")} {patient.fileNo || "—"}
            </div>
          </div>
        </div>
        ...
        <div className="text-start lg:col-span-2 lg:text-end">
          ...
          >
            {t("secretary.patients.viewDetails")}
            {expanded ? (
      ...
      {expanded ? (
        <div className="grid grid-cols-1 gap-4 border-t border-[#EEF2F6] bg-[#F8FAFC] px-4 py-5 text-start sm:grid-cols-2 sm:px-8 lg:grid-cols-4">
          ...
          <div>
            <div className="font-cairo text-[12px] font-bold text-[#98A2B3]">
              {t("secretary.patients.bloodType")}
            </div>
            <div className="font-cairo text-[14px] font-bold text-[#243044]">
              {patient.bloodType || t("secretary.patients.unknown")}
            </div>
          </div>
          <div>
            <div className="font-cairo text-[12px] font-bold text-[#98A2B3]">
              {t("secretary.patients.allergies")}
            </div>
            <div className="font-cairo text-[14px] font-bold text-[#243044]">
          ...
          <div>
            <div className="font-cairo text-[12px] font-bold text-[#98A2B3]">
              {t("secretary.patients.medicalConditions")}
            </div>
            <div className="font-cairo text-[14px] font-bold text-[#243044]">
...
function buildAccountStatusOptions(
  ...
  return [
    { value: "all", label: t("secretary.patients.accountStatus.all") },
    { value: "active", label: t("secretary.patients.accountStatus.active") },
    {
      value: "temporary",
      label: t("secretary.patients.accountStatus.temporary"),
    },
...
export default function SecretaryPatientsPage() {
  ...
  const [accountStatus, setAccountStatus] =
    useState<DoctorPatientAccountStatus>("all");
  ...
  const patients = useMemo<PatientRowData[]>(
    () =>
      (patientsQuery.patients ?? []).map((p) => ({
        id: p._id,
        fileNo: p.publicId,
        name: p.user?.fullName || t("secretary.patients.patient"),
        phone: p.user?.phone || "—",
        email: p.user?.email || "—",
  ...
  const totalPages = Math.max(
    1,
    Math.ceil((patientsQuery.total || patients.length) / limit),
  );
  ...
  return (
    <div
      dir={dir}
      lang={locale}
      className="pb-6 space-y-6 sm:space-y-7 sm:pb-8"
    >
      <SurfaceSection>
        <div className="flex flex-col gap-4 border-b border-[#EEF2F6] px-4 py-5 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8 lg:py-6">
          <div className="text-start">
            <h2 className="font-cairo text-[23px] font-black text-[#243044]">
              {t("secretary.patients.title")}
            </h2>
            <p className="mt-1 font-cairo text-[13px] font-semibold text-[#98A2B3]">
              {(patientsQuery.total || patients.length).toLocaleString(
                numberLocale,
              )}{" "}
              {t("secretary.patients.patients")}
              {searchInput ? t("secretary.patients.matchingSearch") : ""}
              {patientsQuery.isRefetching
                ? t("secretary.patients.refreshingData")
                : ""}
            </p>
          </div>
          ...
          {hasPermission("patients:temporary:create") ? (
            <div className="flex flex-wrap gap-2 items-center">
              ...
              >
                ...
                {t("secretary.patients.addTemporaryPatient")}
              </Link>
            </div>
          ) : null}
        </div>
        ...
        <div className="flex flex-col gap-3 px-4 py-5 sm:px-5 sm:py-6 lg:flex-row lg:items-center">
          ...
          <div className="w-full lg:w-[160px]">
            <StyledSelect
              ...
              options={buildAccountStatusOptions(t)}
              listboxAriaLabel={t("secretary.patients.accountStatus")}
            />
          </div>
          <input
            value={diagnosis}
            ...
            placeholder={t("secretary.patients.diagnosis")}
          ...
          <input
            ...
            aria-label={t("secretary.patients.fromDate")}
          ...
          <input
            ...
            aria-label={t("secretary.patients.toDate")}
            className="h-[40px] w-full rounded-[12px] border border-[#DCE3EC] bg-white px-4 font-cairo text-[14px] font-bold text-[#111827] outline-none focus:border-primary lg:w-[150px]"
          />
        </div>
        ...
        <div className="hidden border-b border-[#EEF2F6] px-8 py-4 lg:block">
          <div className="grid grid-cols-12 gap-4 text-start font-cairo text-[14px] font-bold text-[#A1AAB9]">
            <div className="col-span-4">
              {t("secretary.patients.patientName")}
            </div>
            <div className="col-span-3">
              {t("secretary.patients.phoneNumber")}
            </div>
            <div className="col-span-2">
              {t("secretary.patients.registrationDate")}
            </div>
            <div className="col-span-1">{t("secretary.patients.status")}</div>
            <div className="col-span-2">{t("secretary.patients.actions")}</div>
          </div>
        </div>
        ...
        {!canViewPatients ? (
          ...
          <div className="flex min-h-[220px] flex-col items-center justify-center gap-3 px-4 py-10 text-center sm:px-8">
            ...
            >
              {patientsQuery.isRefetching
                ? t("secretary.patients.retrying")
                : t("secretary.patients.retry")}
            </button>
          </div>
        ) : patients.length === 0 ? (
          <div className="flex min-h-[220px] flex-col items-center justify-center gap-3 px-4 py-10 text-center sm:px-8">
            <p className="font-cairo text-[15px] font-semibold text-[#64748B]">
              {searchInput || diagnosis || from || to || accountStatus !== "all"
                ? t("secretary.patients.noResultsMatch")
                : t("secretary.patients.noPatientsInCategory")}
            </p>
          </div>
        ) : (
          <>
