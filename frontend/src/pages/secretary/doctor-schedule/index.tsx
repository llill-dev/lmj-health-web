...
export default function SecretaryDoctorSchedulePage() {
  ...
  const doctorName =
    ...
    t("secretary.doctorSchedule.assignedDoctor");
  ...
  const dayLabels: Record<string, string> = {
    Sunday: t("secretary.doctorSchedule.sunday"),
    Monday: t("secretary.doctorSchedule.monday"),
    Tuesday: t("secretary.doctorSchedule.tuesday"),
    Wednesday: t("secretary.doctorSchedule.wednesday"),
    Thursday: t("secretary.doctorSchedule.thursday"),
    Friday: t("secretary.doctorSchedule.friday"),
    Saturday: t("secretary.doctorSchedule.saturday"),
  };
        ...
        <div className="px-4 py-5 sm:px-5 sm:py-6 lg:px-8 lg:py-8">
          ...
          {assignedDoctorQuery.isLoading ? (
            <div className="py-8 text-center font-cairo text-[14px] font-semibold text-[#98A2B3]">
              {t("secretary.doctorSchedule.loadingAssignedDoctor")}
            </div>
          ) : assignedDoctorQuery.isError ? (
            <div className="py-8 text-center font-cairo text-[14px] font-semibold text-[#98A2B3]">
              <p>{t("secretary.doctorSchedule.assignedDoctorLoadError")}</p>
              <button
                type="button"
              ...
              >
                {assignedDoctorQuery.isRefetching
                  ? t("secretary.doctorSchedule.retrying")
                  : t("secretary.doctorSchedule.retry")}
              </button>
            </div>
          ) : !doctorId ? (
            <div className="py-8 text-center font-cairo text-[14px] font-semibold text-[#98A2B3]">
              {t("secretary.doctorSchedule.noAssignedDoctor")}
            </div>
          ) : scheduleQuery.isLoading ? (
            <div className="py-8 text-center font-cairo text-[14px] font-semibold text-[#98A2B3]">
              {t("secretary.doctorSchedule.loadingSchedule")}
            </div>
          ) : scheduleQuery.isError ? (
            <div className="py-8 text-center font-cairo text-[14px] font-semibold text-[#98A2B3]">
              <p>{t("secretary.doctorSchedule.scheduleLoadError")}</p>
              <button
                type="button"
            ...
            <div className="space-y-3">
              {scheduleDays.map((schedule) => (
                <ScheduleDayCard
                  ...
                  holidayLabel={t("secretary.doctorSchedule.holiday")}
                  closedLabel={t("secretary.doctorSchedule.closed")}
                  availableLabel={t("secretary.doctorSchedule.available")}
                />
              ))}
            </div>
          )}
