    ...
    <div dir={dir} lang={locale} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-cairo text-2xl font-bold text-[#0f172a]">
            {t("secretary.doctorsDirectory.title")}
          </h1>
          <p className="mt-1 font-cairo text-sm font-medium text-[#64748b]">
            {t("secretary.doctorsDirectory.subtitle")}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex min-w-[200px] flex-1 items-center gap-2 rounded-xl border border-[#e2e8f0] bg-white px-4 py-2.5 shadow-sm">
          ...
          <input
            type="text"
            placeholder={t("secretary.doctorsDirectory.searchPlaceholder")}
            value={search}
        ...
        >
          ...
          {t("secretary.doctorsDirectory.filter")}
          {activeFilterCount > 0 ? (
        ...
        <div className="rounded-xl border border-[#e2e8f0] bg-white p-4 shadow-sm">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            <input
              ...
              placeholder={t("secretary.doctorsDirectory.specialization")}
            ...
            <input
              ...
              placeholder={t("secretary.doctorsDirectory.city")}
              className="h-[38px] rounded-[10px] border border-[#DCE3EC] bg-white px-3 font-cairo text-[13px] font-bold text-[#111827] outline-none focus:border-primary"
            />
            <input
              value={country}
              onChange={(event) => setCountry(event.target.value)}
              placeholder={t("secretary.doctorsDirectory.country")}
              className="h-[38px] rounded-[10px] border border-[#DCE3EC] bg-white px-3 font-cairo text-[13px] font-bold text-[#111827] outline-none focus:border-primary"
            />
            <select
            ...
            >
              {CONSULTATION_TYPE_OPTIONS.map((option) => (
                <option key={option.value} value={option.value}>
                  {t(`secretary.doctorsDirectory.${option.key}`)}
                </option>
              ))}
            </select>
            <input
              ...
              placeholder={t("secretary.doctorsDirectory.minRating")}
              className="h-[38px] rounded-[10px] border border-[#DCE3EC] bg-white px-3 font-cairo text-[13px] font-bold text-[#111827] outline-none focus:border-primary"
            />
          </div>
          {activeFilterCount > 0 ? (
            ...
            >
              ...
              {t("secretary.doctorsDirectory.clearFilters")}
            </button>
          ) : null}
        </div>
      ) : null}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {!canViewDirectory ? (
          ...
          <div className="col-span-full space-y-3 py-8 text-center">
            ...
            >
              {t("secretary.doctorsDirectory.retry")}
            </button>
          </div>
        ) : directoryQuery.isAwaitingData ? (
          <div className="col-span-full py-8 text-center font-cairo text-sm font-semibold text-[#64748b]">
            {t("secretary.doctorsDirectory.loading")}
          </div>
        ) : doctors.length === 0 ? (
          <div className="col-span-full py-8 text-center font-cairo text-sm font-semibold text-[#64748b]">
            {t("secretary.doctorsDirectory.noResults")}
          </div>
        ) : (
          <>
            {directoryQuery.isRefetching ? (
              <div className="col-span-full text-start font-cairo text-xs font-semibold text-[#64748b]">
                {t("secretary.doctorsDirectory.refreshingData")}
              </div>
            ) : null}
            {doctors.map((doctor) => (
              <div
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-1 flex items-center gap-1">
                      ...
                      <span className="font-cairo text-xs font-medium text-[#64748b]">
                        ({doctor.reviews}{" "}
                        {t("secretary.doctorsDirectory.reviews")})
                      </span>
                    </div>
                  </div>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-cairo text-sm text-[#0f172a]">
                      {doctor.city ||
                        t("secretary.doctorsDirectory.notSpecified")}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
      ...
      directoryQuery.total > 0 ? (
        <MedicalRecordsPagination
          ...
          pageSize={pageSize}
          itemLabel={t("secretary.doctorsDirectory.doctor")}
