...
export default function RestoreAccountPage() {
  ...
  const profileSettingsPath =
    role === 'patient' ? '/patient/profile-settings' : '/doctor/profile-settings';
  ...
  if (!authUser && !pendingRecovery) {
    return (
      <Navigate
        to={`/login?next=${encodeURIComponent(restorePath)}`}
        replace
      />
    );
  }
  ...
  const executePatientRestore = async () => {
    ...
    try {
      ...
      setError(
        mapAccountDeletionGenericError(cause, 'تعذّر إلغاء طلب الحذف.'),
      );
    } finally {
      setBusy(false);
    }
      ...
      <DeleteAccountShell step={1}>
        {view === 'otp' ? (
          <DeleteAccountOtpStep
            ...
            error={error}
            title={isRestoreRequestMode ? 'طلب استعادة الحساب' : 'استعادة الحساب'}
            subtitle={
              isRestoreRequestMode
          ...
          <div className="text-center">
            ...
            {!loading && !canRestore && resolvedDeletionStatus !== 'none' ? (
              <p className="mt-6 font-cairo text-[13px] font-semibold text-[#DC2626]">
                لا يمكن استعادة هذا الحساب حالياً. تواصل مع الدعم إذا كنت تعتقد
                أن هذا خطأ.
              </p>
            ) : null}
            ...
            >
              {authUser ? 'الرجوع إلى الملف الشخصي ←' : 'العودة لتسجيل الدخول ←'}
            </Link>
          </div>
        )}
      </DeleteAccountShell>
