...
export default function AdminComplaintDetailsPage() {
  ...
  const nextActionLabel = (status: ComplaintLifecycleStatus) => {
    if (status === "submitted")
      return t("admin.complaints.details.nextActionStartReview");
    if (status === "under_review")
      return t("admin.complaints.details.nextActionMoveToProcessing");
    if (status === "in_progress")
      return t("admin.complaints.details.nextActionResolveOrClose");
    if (status === "resolved")
      return t("admin.complaints.details.currentActionReviewClosure");
    return t("admin.complaints.details.closedForReference");
  };
  ...
  const detailErrorMessage = detailQuery.isError
    ? complaintUserFacingError(
        detailQuery.error,
        t("admin.complaints.details.failedToLoadDetails"),
      )
    : null;
  const updateErrorMessage = updateMutation.isError
    ? complaintUserFacingError(
        updateMutation.error,
        t("admin.complaints.details.failedToUpdateStatus"),
      )
    : null;
  ...
  async function openAttachment(
    ...
    if (!complaintId || !fid) {
      toast(t("admin.complaints.details.failedToFindAttachment"), {
        title: t("admin.complaints.details.failedToOpenAttachment"),
        variant: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (!url) {
        toast(t("admin.complaints.details.serverNoValidLink"), {
          title: t("admin.complaints.details.failedToOpenAttachment"),
          variant: "error",
        });
        return;
      ...
      const filename =
        res.originalName ||
        attachment.label?.replace(/\s+/g, "_") ||
        "attachment";
      if (mode === "view") {
        await triggerBrowserFileDownloadAndOpen(url, filename);
        toast(t("admin.complaints.details.attachmentOpened"), {
          title: t("admin.complaints.details.attachmentOpenedTitle"),
          variant: "success",
          durationMs: 2600,
        });
      } else {
        await triggerBrowserFileDownload(url, filename);
        toast(t("admin.complaints.details.downloadStarted"), {
          title: t("admin.complaints.details.downloadStartedTitle"),
          variant: "success",
          durationMs: 2600,
        });
      }
      ...
      toast(
        complaintUserFacingError(
          error,
          mode === "view"
            ? t("admin.complaints.details.failedToOpen")
            : t("admin.complaints.details.failedToDownload"),
        ),
        {
          title:
            mode === "view"
              ? t("admin.complaints.details.failedToOpenAttachment")
              : t("admin.complaints.details.failedToDownloadAttachment"),
          variant: "error",
          durationMs: 4200,
        },
      );
  ...
  return (
    <>
      <Helmet>
        <title>{t("admin.complaints.details.page.title")} • LMJ Health</title>
      </Helmet>
      ...
      <div dir={dir} lang={locale} className="text-start">
        ...
        >
          ...
          {t("admin.complaints.details.backToComplaints")}
        </button>
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
              <div className="rounded-[10px] border border-[#E5E7EB] bg-[#F8FAFC] px-4 py-3">
                <div className="mb-2 font-cairo text-[11px] font-bold text-[#667085]">
                  {t("admin.complaints.details.requestType")}
                </div>
                <div className="font-cairo text-[13px] font-extrabold text-[#111827]">
                  {t("admin.complaints.details.userComplaint")}
                </div>
              </div>
              <div className="rounded-[10px] border border-[#E5E7EB] bg-[#F8FAFC] px-4 py-3">
                <div className="mb-2 font-cairo text-[11px] font-bold text-[#667085]">
                  {t("admin.complaints.details.complaintType")}
                </div>
                <div className="font-cairo text-[13px] font-extrabold text-[#111827]">
              ...
              <div className="rounded-[10px] border border-[#E5E7EB] bg-[#F8FAFC] px-4 py-3">
                <div className="mb-2 font-cairo text-[11px] font-bold text-[#667085]">
                  {t("admin.complaints.details.currentAction")}
                </div>
                <div className="font-cairo text-[13px] font-extrabold text-[#111827]">
                  {nextActionLabel(c.status).replace(
                    t("admin.complaints.details.nextActionPrefix"),
                    "",
                  )}
                </div>
              </div>
            ...
            <div className="rounded-[10px] border border-[#D6EEEC] bg-[#F3FBFA] px-4 py-4 sm:px-5">
              <p className="font-cairo text-[13px] font-semibold leading-6 text-[#215A57]">
                {c.status === "closed"
                  ? t("admin.complaints.details.closedInfo")
                  : c.status === "resolved"
                    ? t("admin.complaints.details.resolvedInfo")
                    : t("admin.complaints.details.activeInfo")}
              </p>
            </div>
            ...
            <article className="overflow-hidden rounded-[10px] border-[1.5px] border-[#E8ECF2] bg-white shadow-[0_10px_28px_rgba(15,23,42,0.06)]">
              <div className="p-6 sm:p-8">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex-1 min-w-0 text-start">
                      ...
                      <div className="mt-2 font-cairo text-[16px] font-semibold leading-snug sm:text-[17px]">
                        <span className="text-primary">
                          {t("admin.complaints.details.complaintTypeLabel")}
                        </span>
                  ...
                  <div className="shrink-0 font-cairo text-[15px] font-black text-[#99A1AF] sm:pt-1">
                    {formatHeaderTime(
                      c.createdAt,
                      locale,
                      t("admin.complaints.details.today"),
                    )}
                  </div>
                </div>
                ...
                <h2 className="mb-4 font-cairo text-[16px] font-black text-[#111827]">
                  {t("admin.complaints.details.attachedFiles")}
                </h2>
                ...
                <div className="mb-4 rounded-[10px] border border-[#D5E8E6] bg-[#F8FFFE] px-4 py-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="font-cairo text-[12px] font-semibold leading-6 text-[#5B7B79]">
                      {t("admin.complaints.details.attachmentDisclaimer")}
                    </div>
                  </div>
                </div>
                ...
                {!hasAttachments ? (
                  <p className="rounded-lg border border-dashed border-[#E5E7EB] bg-[#F9FAFB] px-4 py-6 text-center font-cairo text-[13px] font-semibold text-[#94A3B8]">
                    {t("admin.complaints.details.noAttachedFiles")}
                  </p>
                ) : (
                  <div className="flex flex-col gap-3 rounded-[10px] border border-[#D6EEEC] bg-[#F3FBFA] p-3 sm:p-4">
                    {attachments.length === 0 ? (
                      <p className="font-cairo text-[13px] text-[#64748B]">
                        {t("admin.complaints.details.attachmentsNotLoaded")}
                      </p>
                    ) : (
                      attachments.map((att) => (
                        <div
                        ...
                        >
                          ...
                          <div className="flex gap-2 items-center shrink-0">
                            <button
                              type="button"
                              disabled={fileActionId === `${att.fileId}-view`}
                              ...
                              aria-label={t(
                                "admin.complaints.details.viewFile",
                              )}
                            >
                            ...
                            <button
                              ...
                              className="flex h-10 w-10 items-center justify-center rounded-[8px] bg-[#148283] text-white shadow-sm transition hover:brightness-110 disabled:opacity-50"
                              aria-label={t(
                                "admin.complaints.details.downloadFile",
                              )}
                            >
                              {fileActionId === `${att.fileId}-download` ? (
                                <Loader2 className="w-5 h-5 animate-spin" />
              ...
              {c.statusHistory?.length ? (
                <section className="rounded-[10px] border border-[#E8ECF2] bg-white p-5 shadow-sm">
                  <h2 className="mb-3 font-cairo text-[15px] font-black text-primary">
                    {t("admin.complaints.details.statusHistory")}
                  </h2>
              ...
              {c.adminResponse ? (
                <section className="rounded-[10px] border border-emerald-200 bg-emerald-50/60 p-5">
                  <h2 className="mb-2 font-cairo text-[14px] font-black text-emerald-900">
                    {t("admin.complaints.details.adminResponse")}
                  </h2>
            ...
            <section className="rounded-[10px] border border-[#E8ECF2] bg-white p-5 shadow-sm sm:p-6">
              <h2 className="mb-1 font-cairo text-[15px] font-black text-primary">
                {t("admin.complaints.details.updateStatus")}
              </h2>
              <p className="mb-4 font-cairo text-[12px] text-[#64748B]">
                {t("admin.complaints.details.currentStatusLabel")}{" "}
                <strong>{statusLabelAr(c.status, locale)}</strong>.{" "}
                {t("admin.complaints.details.statusTransitionValidated")}
              </p>
              <div className="flex flex-col gap-3 max-w-xl">
                <label className="block">
                  <span className="mb-1 block font-cairo text-[12px] font-bold text-[#475467]">
                    {t("admin.complaints.details.newStatus")}
                  </span>
                  <StyledSelect
                    value={nextStatus}
                    onChange={(v) =>
                    ...
                    placeholder={t("admin.complaints.details.select")}
                    options={STATUS_OPTIONS.map((s) => ({
                      value: s,
                      label: statusLabelAr(s, locale),
                    }))}
                    listboxAriaLabel={t(
                      "admin.complaints.details.newStatusAriaLabel",
                    )}
                  />
                </label>
                <label className="block">
                  <span className="mb-1 block font-cairo text-[12px] font-bold text-[#475467]">
                    {t("admin.complaints.details.adminResponseLabel")}
                  </span>
                  <textarea
                    value={adminResponse}
                    ...
                    placeholder={t("admin.complaints.details.replyPlaceholder")}
                  />
                </label>
                ...
                >
                  {updateMutation.isPending ? (
                    ...
                    t("admin.complaints.details.saveUpdate")
                  )}
                </button>
              </div>
            </section>
      ...
      <ConfirmActionDialog
        ...
        variant="primary"
        title={t("admin.complaints.details.confirmUpdate")}
        icon={<Save className="h-6 w-6" strokeWidth={2} aria-hidden />}
        description={
          nextStatus ? (
            <>
              {t("admin.complaints.details.statusChangeFrom")}{" "}
              <span className="font-extrabold text-[#344054]">
                {statusLabelAr(c?.status as ComplaintLifecycleStatus, locale)}
              </span>
              {t("admin.complaints.details.statusChangeTo")}{" "}
              <span className="font-extrabold text-[#344054]">
                {statusLabelAr(nextStatus, locale)}
              </span>
              {t("admin.complaints.details.statusChangeEnd")}
              {adminResponse.trim()
                ? t("admin.complaints.details.sendReply")
                : "."}
            </>
          ) : (
            t("admin.complaints.details.selectStatusFirst")
          )
        }
        confirmLabel={t("admin.complaints.details.confirmSave")}
        ...
        successToast={{
          title: t("admin.complaints.details.updated"),
          message: t("admin.complaints.details.updateSuccess"),
          variant: "success",
        }}
      />
    </>
