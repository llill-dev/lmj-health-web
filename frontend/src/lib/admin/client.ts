...
import type {
  ...
  ComplaintAttachmentDownloadUrlResponse,
  ApiSuccessEnvelope,
...
export const adminApi = {
  ...
  complaints: {
    ...
    // Complaint-scoped attachment download (admin-only). Verifies the file
    // is actually attached to this complaint server-side; never falls back
    // to the generic patient-files download route.
    getAttachmentDownloadUrl: (complaintId: string, fileId: string) =>
      get<ComplaintAttachmentDownloadUrlResponse>(
        adminEndpoints.complaints.attachmentDownloadUrl(complaintId, fileId),
        { locale: "ar" },
      ),
  },
  ...
  medicalOrderCatalog: {
    list: async (params: AdminMedicalOrderCatalogListParams) => {
      ...
      try {
        ...
        >(endpoint, { locale: "ar" });
        const items = normalizeMedicalOrderCatalogList(raw);
        const body = asAdminRecord(raw) ?? {};
        return { items, ...readPagedNumbers(body, items.length) };
      } catch (e) {
        if (
          import.meta.env.DEV &&
          ...
          const items = DEV_MEDICAL_ORDER_PLACEHOLDERS[params.type];
          return {
            items,
            page: 1,
            limit: items.length,
