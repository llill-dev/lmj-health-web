...
export const adminEndpoints = {
  ...
  complaints: {
    ...
    // Complaint-scoped attachment download (admin-only). Never reuse the
    // generic /api/patients/:patientId/files/:fileId/download route here.
    attachmentDownloadUrl: (complaintId: string, fileId: string) =>
      `/api/complaints/${complaintId}/attachments/${fileId}/download-url`,
  },
  content: {
  ...
  /** Fixed read-only blood-type enum — never sourced from /admin/lookups. */
  meta: {
    healthProfileOptions: "/api/meta/health-profile-options",
  },
