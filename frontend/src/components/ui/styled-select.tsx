...
function StyledSelectInner(
  ...
  const updateListboxPosition = useCallback(() => {
    ...
    if (portalRect) {
      mode = "portal";
      availBelowPx = Math.max(0, portalRect.bottom - rect.bottom - edgePad - gap);
      ...
      left = Math.max(edgePad, Math.min(rect.left, vw - clampedWidth - edgePad));
    }

    if (availAbovePx >= availBelowPx) {
      placement = "above";
      innerMaxHeight = Math.min(dropdownMaxHeight, Math.max(floorPx, availAbovePx));
    } else {
      placement = "below";
      innerMaxHeight = Math.min(dropdownMaxHeight, Math.max(floorPx, availBelowPx));
    }

    innerMaxHeight = Math.floor(innerMaxHeight);

    if (mode === "portal" && portalRect) {
      top = placement === "below" ? rect.bottom - portalRect.top + gap : undefined;
      bottom = placement === "above" ? portalRect.bottom - rect.top + gap : undefined;
    } else {
      top = placement === "below" ? rect.bottom + gap : undefined;
      bottom = placement === "above" ? Math.max(edgePad, vh - rect.top + gap) : undefined;
    }
  ...
  const emphasisBg = tone === "emphasis" ? "bg-primary/10 text-primary" : "bg-white";
          ...
          open && !mergedDisabled && listboxPos ? (
            <motion.div
              ...
              className={[
                ...
                listboxPos.placement === "below" ? "origin-top" : "origin-bottom",
              ].join(" ")}
              style={{
                zIndex:
                  listboxPos.mode === "viewport" ? listboxZIndex : undefined,
                transformOrigin:
                  listboxPos.placement === "below" ? "top center" : "bottom center",
                top:
                  listboxPos.placement === "below" ? listboxPos.top : undefined,
                bottom:
                  listboxPos.placement === "above" ? listboxPos.bottom : undefined,
