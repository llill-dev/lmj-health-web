...
const requestSchema = z
  .object({
    ...
    identifier: z.string().min(1, "auth.resetPassword.fieldRequired"),
  })
  .superRefine((val, ctx) => {
    if (val.method === "email") {
    ...
    if (val.method === "phone") {
      if (!isValidAuthPhoneIdentifier(val.identifier)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["identifier"],
          message: "auth.resetPassword.invalidPhone",
        });
      }
    }
  });
...
export default function ForgotPasswordRequest({
  onBack,
  onSubmit,
  submitLabel = "auth.resetPassword.sendCode",
  defaultMethod = "email",
  defaultIdentifier = "",
  variant = "reset",
  title,
  ...
  const methodLabel = useMemo(
    () =>
      method === "phone"
        ? t("auth.resetPassword.phoneMethod")
        : t("auth.resetPassword.emailMethod"),
    [method, t],
  );
  ...
  const sectionTitle = useMemo(
    () =>
      method === "phone"
        ? t("auth.resetPassword.enterPhone")
    ...
    <>
      <div className="text-center">
        ...
        <p className="mt-1 font-cairo text-[13px] font-semibold text-[#667085]">
          {t("auth.resetPassword.verifyIdentity")}
        </p>
      </div>
      ...
      <form onSubmit={submit} className="mt-5" noValidate>
        ...
        >
          <span>
            {isSubmitting ? t("auth.resetPassword.sending") : t(submitLabel)}
          </span>
  ...
  if (variant === "plain") {
    return (
      <section
        dir={dir}
        lang={locale}
        className="mx-auto flex w-full max-w-[520px] flex-col items-center px-4"
      >
  ...
  return (
    <PasswordResetShell step={1} title={title} subtitle={subtitle}>
      {formBody}
    </PasswordResetShell>
  );
}
