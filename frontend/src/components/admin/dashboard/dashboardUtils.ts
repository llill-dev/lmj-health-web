...
const COMPLAINT_STATUS_LABELS: Record<
  ComplaintLifecycleStatus,
...
const CONTENT_STATUS_LABELS: Record<
  AdminContentStatus,
  { label: string; className: string }
> = {
  DRAFT: {
...
const CONTENT_TYPE_LABELS: Record<AdminContentType, string> = {
  CONDITION: "الحالات",
  SYMPTOM: "الأعراض",
...
const DEFAULT_CONTENT_STATUS_LABEL = {
  label: "—",
  className: "bg-[#E5E7EB] text-[#374151]",
};
...
export function complaintTypeLabel(
  ...
  if (type && type in COMPLAINT_TYPE_LABELS)
    return COMPLAINT_TYPE_LABELS[type as ComplaintType];
...
export function complaintStatusLabel(
  ...
  if (status && status in COMPLAINT_STATUS_LABELS) {
    return COMPLAINT_STATUS_LABELS[status as ComplaintLifecycleStatus];
  }
  return {
...
export function contentStatusLabel(
  ...
  if (status && status in CONTENT_STATUS_LABELS) {
    return CONTENT_STATUS_LABELS[status as AdminContentStatus];
  }
  return DEFAULT_CONTENT_STATUS_LABEL;
}

export function contentTypeCategoryLabel(
  type: AdminContentType | string | undefined,
): string {
  if (type && type in CONTENT_TYPE_LABELS)
    return CONTENT_TYPE_LABELS[type as AdminContentType];
  return type ? String(type) : "—";
}
