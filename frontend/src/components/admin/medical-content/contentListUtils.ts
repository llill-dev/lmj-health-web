...
export function resolvePagedLimit(
  ...
  return (
    toPositiveInt(payload?.limit) ?? Math.max(1, Math.floor(fallbackLimit))
  );
}
...
export function normalizeItemLanguage(raw: unknown): "ar" | "en" | "unknown" {
  ...
  const s = String(raw).trim().toLowerCase().normalize("NFKC");
          ...
          const normalizeActor = (
            ...
            raw && typeof raw === "object"
              ? {
                  ...
                  email: toDisplayText((raw as Record<string, unknown>).email),
                }
              : toDisplayText(raw);
...
export function languageKindLabel(
  ...
  if (n === "ar")
    return { code: "ar", label: tt(locale, "adminContent.language.arabic") };
...
export function contentTypeLabel(
  ...
  return (
    getTranslationValue(locale, `adminContent.type.${t}`) ??
    tt(locale, "adminContent.type.general")
  );
}
