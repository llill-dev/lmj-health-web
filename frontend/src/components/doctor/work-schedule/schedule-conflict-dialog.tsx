...
export default function ScheduleConflictDialog({
  ...
  const operationLabels = {
    update_day: 'تحديث جدول اليوم',
    delete_day: 'حذف اليوم',
    replace_weekly_schedule: 'استبدال الجدول الأسبوعي',
    update_slot: 'تحديث الفترة الزمنية',
  };
  ...
  return (
    <ConfirmDialog
      ...
      title="تعارض في المواعيد المحجوزة"
      description={`لا يمكن ${operationLabels[data.operation]} لأن هناك ${data.totalConflicts} ${
      ...
      {data.nextAction && (
        <div className='mt-4 rounded-[10px] border border-blue-200 bg-blue-50 p-3 text-right'>
          <p className='font-cairo text-[13px] font-semibold text-blue-900'>
            <span className='font-extrabold'>الإجراء المطلوب:</span>{' '}
            {data.nextAction}
          </p>
        </div>
      )}
