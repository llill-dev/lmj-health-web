...
export default function ScheduleConflictDialog({
  ...
  const operationLabels = {
    update_day: t("doctor.schedule.conflict.operation.updateDay"),
    delete_day: t("doctor.schedule.conflict.operation.deleteDay"),
    replace_weekly_schedule: t(
      "doctor.schedule.conflict.operation.replaceWeeklySchedule",
    ),
    update_slot: t("doctor.schedule.conflict.operation.updateSlot"),
  };

  const conflictNoun =
    data.totalConflicts === 1
      ? t("doctor.schedule.conflict.conflictNoun.singular")
      : data.totalConflicts === 2
        ? t("doctor.schedule.conflict.conflictNoun.dual")
        : t("doctor.schedule.conflict.conflictNoun.plural");
  ...
  return (
    <ConfirmDialog
      ...
      title={t("doctor.schedule.conflict.title")}
      ...
      customActions={
        <div className="flex flex-row-reverse gap-3">
          ...
          >
            {t("doctor.schedule.conflict.cancel")}
          </button>
          ...
          {onViewAppointments && (
            ...
            >
              ...
              {t("doctor.schedule.conflict.viewAppointments")}
            </button>
          )}
          ...
          {onForceUpdate && (
            ...
            >
              {t("doctor.schedule.conflict.applyChange")}
            </button>
          )}
        </div>
      }
    >
      <h4 className="mb-3 text-start font-cairo text-[14px] font-extrabold text-[#111827]">
        {t("doctor.schedule.conflict.conflictingAppointments")}
      </h4>
            ...
            <div className="flex-1 text-start">
              <div className="flex items-center justify-end gap-2">
                <span className="font-cairo text-[13px] font-extrabold text-[#111827]">
                  {t("doctor.schedule.conflict.appointment")} #{index + 1}
                </span>
              </div>
        ...
        <div className="mt-4 rounded-[10px] border border-blue-200 bg-blue-50 p-3 text-start">
          <p className="font-cairo text-[13px] font-semibold text-blue-900">
            <span className="font-extrabold">
              {t("doctor.schedule.conflict.requiredAction")}
            </span>{" "}
            {data.nextAction}
          </p>
        </div>
