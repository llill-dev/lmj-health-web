...
function buildClinicalActions(t: TFn): Array<{
  ...
  return [
    {
      key: 'lab',
      label: t('doctor.consultationReplyPanel.actions.lab'),
      icon: FlaskConical,
    },
    {
      key: 'imaging',
      label: t('doctor.consultationReplyPanel.actions.imaging'),
      icon: ScanLine,
    },
    {
      key: 'prescription',
      label: t('doctor.consultationReplyPanel.actions.prescription'),
      icon: Pill,
    },
  ];
}
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      const response = await doctorApi.patients.uploadFile(
        patientId,
        file,
        t('doctor.consultationReplyPanel.uploadAttachmentName'),
      );
      ...
      toast(t('doctor.consultationReplyPanel.fileAttachedToast'), {
        title: t('doctor.consultationReplyPanel.uploadFileTitle'),
        variant: 'success',
      });
    } catch (error) {
  ...
  return (
    <div className="mt-4 rounded-[12px] border border-[#EEF2F6] bg-white px-4 py-4">
      <div className="font-cairo text-[12px] font-extrabold text-[#111827]">
        {t('doctor.consultationReplyPanel.sendReply')}
      </div>
      ...
      <div dir="ltr" className="mt-3 flex items-center gap-2">
        <button
          ...
          disabled={!canSend}
          onClick={onSend}
          aria-label={t('doctor.consultationReplyPanel.sendReplyAria')}
        ...
        <input
          ...
          placeholder={
            disabled
              ? t('doctor.consultationReplyPanel.disabledPlaceholder')
              : t('doctor.consultationReplyPanel.replyPlaceholder')
          }
        ...
        <button
          ...
          aria-label={t('doctor.consultationReplyPanel.uploadFile')}
          title={t('doctor.consultationReplyPanel.uploadFile')}
          ...
          {pendingAttachments.map((attachment) => (
            ...
            >
              ...
              <button
                ...
                aria-label={t('doctor.consultationReplyPanel.removeAttachment')}
              >
      ...
      <div dir={dir} className="mt-2">
        ...
        >
          {pickerOpen
            ? t('doctor.consultationReplyPanel.hidePatientFiles')
            : t('doctor.consultationReplyPanel.choosePatientFile')}
        </button>
      </div>
              ...
              return (
                  ...
                  onClick={() => {
                    addAttachment({
                      ...
                      fileName:
                        ...
                        t('doctor.consultationReplyPanel.patientFileFallback'),
                    });
                    setPickerOpen(false);
                  }}
                ...
                >
                  <span className="truncate">
                    {file.originalName || t('doctor.consultationReplyPanel.unnamedFile')}
                  </span>
                  {selected ? (
                    <span className="text-[11px] font-extrabold text-primary">
                      {t('doctor.consultationReplyPanel.added')}
                    </span>
                  ) : null}
                </button>
              );
      ...
      <div dir={dir} className="mt-4 grid grid-cols-3 gap-2">
        {buildClinicalActions(t).map((action) => {
      ...
      {!disabled ? (
        <p dir={dir} className="mt-2 font-cairo text-[10px] font-semibold text-[#98A2B3]">
          {t('doctor.consultationReplyPanel.autoLinkNote')}
        </p>
      ) : null}
      ...
      {!disabled ? (
        <div dir={dir} className="mt-4 grid grid-cols-2 gap-3">
          <button
            type="button"
            disabled
            title={t('doctor.consultationReplyPanel.draftUnsupportedTitle')}
          ...
          >
            ...
            {t('doctor.consultationReplyPanel.saveAsDraft')}
          </button>
          <button
            type="button"
            disabled={closing}
          ...
          >
            ...
            {t('doctor.consultationReplyPanel.closeConsultation')}
          </button>
        </div>
      ) : null}
      ...
      {!disabled ? (
        ...
        >
          ...
          {t('doctor.consultationReplyPanel.rejectConsultation')}
        </button>
      ) : null}
    </div>
  );
