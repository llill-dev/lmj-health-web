...
export function EncounterDocumentsPanel({
  ...
  const handleLink = async (body: EncounterDocumentLinkBody, label: string) => {
    try {
      ...
      toast(t('doctor.encounterDocuments.linked').replace('{label}', label), { variant: 'success' });
    } catch (error) {
  ...
  const handleShare = async (documentId: string, title: string) => {
    const shareNote = window.prompt(t('doctor.encounterDocuments.shareNotePrompt')) ?? undefined;
    try {
      await mutations.shareDocument({
        documentId,
        shareNote: shareNote?.trim() || undefined,
      });
      toast(t('doctor.encounterDocuments.shared').replace('{title}', title), { variant: 'success' });
    } catch (error) {
      toast(getUserFacingRequestErrorMessage(error), { variant: 'error' });
    }
  };
    ...
    >
      <div className="mb-4 flex items-center gap-2">
        ...
        <h2 className="font-cairo text-[16px] font-black text-[#111827]">
          {t('doctor.encounterDocuments.title')}
        </h2>
      </div>
      ...
      {linkCandidates.length > 0 ? (
        <div className="mb-4 flex flex-wrap gap-2">
          {linkCandidates.map((candidate) => (
            ...
            >
              {t('doctor.encounterDocuments.link').replace('{label}', candidate.label)}
            </button>
          ))}
        </div>
      ) : null}
      ...
      {documentsQuery.isAwaitingData ? (
        <p className="font-cairo text-[13px] font-semibold text-[#667085]">
          {t('doctor.encounterDocuments.loading')}
        </p>
      ) : documentsQuery.documents.length === 0 ? (
        <p className="font-cairo text-[13px] font-semibold text-[#98A2B3]">
          {t('doctor.encounterDocuments.empty')}
        </p>
      ) : (
        <ul className="space-y-3">
          {documentsQuery.documents.map((document) => {
            const documentId = document._id ?? document.id ?? '';
            const title = document.title?.trim() || t('doctor.encounterDocuments.defaultTitle');
            return (
              <li
                key={documentId}
              ...
              >
                <div className="min-w-0 text-start">
                  ...
                  <p className="mt-1 font-cairo text-[11px] font-semibold text-[#667085]">
                    {document.sharedWithPatient
                      ? t('doctor.encounterDocuments.sharedAt').replace(
                          '{date}',
                          document.sharedAt
                      ...
                      : t('doctor.encounterDocuments.notShared')}
                  </p>
                </div>
                {!document.sharedWithPatient && documentId ? (
                  ...
                  >
                    ...
                    {t('doctor.encounterDocuments.share')}
                  </button>
                ) : null}
              </li>
            );
