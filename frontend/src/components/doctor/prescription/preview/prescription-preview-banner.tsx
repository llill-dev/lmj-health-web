...
export function PrescriptionPreviewBanner({
  ...
  const subtitle = loading
    ? t('doctor.prescriptionPreview.loadingPatientData')
    : buildPrescriptionPatientSubtitle(patientName, t);
  const resolvedStatusLabel = statusLabel ?? t('doctor.prescription.defaultStatus');
          ...
          <div className="min-w-0">
            ...
            {!loading && patientName?.trim() ? (
              <p className="mt-0.5 font-cairo text-[13px] font-semibold leading-[20px] text-primary/75">
                {t('doctor.prescriptionPreview.confirmBeforeSending')}
              </p>
            ) : null}
          </div>
        </div>
